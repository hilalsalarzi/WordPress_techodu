...
class UniteCreatorWooIntegrate{
	...
	public function getCartHtml(){
		...
		$html = "<div class='uc-minicart-items'>";
		...
		foreach($arrCartItems as $cart_item_key=>$cart_item){
			...
			$htmlItem = "<span class=\"uc-minicart-item\" data-key=\"{$cart_item_key}\">
				<pre>
				$item_name ($quantity) - $priceHtml <a class='remove-from-cart' style='color:blue' href='javascript:void(0)'>X</a> 
				</pre>
				</span>
			";
	...
	public static function onAddCartFragment($fragment){
		...
		$html = $objWoo->getCartHtml();
		
		$fragment[".uc-minicart-items"] = $html;
