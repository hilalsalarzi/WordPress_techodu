...
if ( !class_exists( 'ContentViews_Block' ) ) {
	...
	class ContentViews_Block {
		...
		function mapping( $data, &$settings ) {
			...
			foreach ( $atts as $block_key => $info ) {
				if ( isset( $info[ '__key' ] ) ) {
					...
					$settings[ PT_CV_PREFIX . $key ] = isset( $data[ $block_key ] ) ? $data[ $block_key ] : '';
				}
			}
		...
		static function get_fields() {
			return [ 'view', 'remain-wrapper', 'overlay-wrapper', 'content-item', 'meta-fields', 'heading', 'thumbnail', 'thumbnailsm', 'title', 'titlesm', 'content', 'readmore', 'taxoterm', 'pagination', 'paginationActive', 'wooprice', 'wooatc' ];
		}
